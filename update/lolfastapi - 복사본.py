# Pentakill Updator Module
#
# This module opens socket and connects to
# League of Legends official API and works as a wrapper for it
#
# All API terminals such as game history, match, league, summoner
# are handled by methods of Requestor object
#
# LOLAdmin object distributes an instance of LOLWorker
# LOLWorker object actually opens socket and gets stream of data
# Each thread makes use of one LOLWorker instance
# LOLAdmin manages all instances it generated so that total request number
# does not exceed the limitation
#
# When total limit is exceeded, LOLAdmin will not allow its LOLWorkers to
# request to LOL API server. When limit is reset, LOLAdmin will let workers
# start again.
#
# LOLAdmin uses one key in key.py in the same directory
...
class _TimerEventManager(object):
    ...
    def _timer_func_factory(method, n):
        def timer_start():
            method(id=n)
        return timer_start
    ...
    def reserve_timer(self, name, method, time):
        ...
        while True:
            ...
            if meta[3]:
                break
            meta[0] += 1
            timer_func = self._timer_func_factory(method, meta[0])
...
class LOLAdmin(object):
    ...
    def __init__(self, limit=None, key=None, cores=None, api=None):
        ...
        # timer for service available
        self.available_thread = None
        
        # policy for service unavailable
        self.policy = _ServiceUnavailablePolicy()
        # timer for service unavailable
        self.unavailable_thread = None
        ...
        # Mutex for timer starting
        self.timer_mutex = threading.Semaphore(1)
    ...
    def _synchronize(self):
        ...
        # sync_mutex -> perm_condition -> req_condition -> timer_mutex -> sync_miss_mutex
        # otherwise, deadlock may occur
        
        start = self.state.start_event([S_OK])
        if not start[0]:
            self.timer_mutex.acquire()
            self.sync_thread = None
            self.timer_mutex.release()
            return
        ...
        # it may be possible that rate reset thread was started
        # but sync first got sync_mutex
        if self.rate_timer:
    ...
    def _start_service_available(self):
        if self.available_thread == None:
            self.available_thread = threading.Timer(0.0, self._service_available)
            self.available_thread.start()
    ...
    def _call_service_unavailable(self):
        self.timer_mutex.acquire()
        self._start_service_unavailable()
        self.timer_mutex.release()
        
    def _start_service_unavailable(self):
        if self.unavailable_thread == None:
            self.unavailable_thread = threading.Timer(0.0, self._service_unavailable)
            self.unavailable_thread.start()
    ...
    def _call_synchronize(self):
        ...
        self._start_synchronize()
        self.timer_mutex.release()
            
    def _start_synchronize(self):
        if self.sync_thread == None:
            self.sync_thread = threading.Timer(0.0, self._synchronize)
            self.sync_thread.start()
            
    # sets timer for rate reset
    # time : time untile timer is called in sec, if time is not given, self.per_time is used
    # overlay : if True, set new timer even if rate_timer is not None, we don't cancel old timer.
    #          this means caller should be timer routine
    #          otherwise, set timer only when rate_timer is None, default is False 
    def _call_reset_timer(self, time=None, overlay=False):
        self.timer_mutex.acquire()
        time = self.per_time if time == None else time
        if overlay:
            self.rate_timer = None
        self._start_timer(time)
        self.timer_mutex.release()
        
    # set timer if no timer is reserved
    def _start_timer(self, time):
        if self.rate_timer == None:
            self._debug_msg('set timer')
            self.rate_timer = threading.Timer(time, self._reset_rate)
    ...
    def _reset_rate(self):
        ...
        while True:
            # it's possible that _reset_rate thread is started and 
            # sync gets sync_mutex first. then, no reset.
            cur_time = time.time()
            #print 'Time elapsed after last sync' , cur_time - self.sync_last
            if self.sync_last and cur_time - self.sync_last < self.per_time:
                self._call_reset_timer(self.per_time - (cur_time - self.sync_last), True)
                self.timer_mutex.release()
                break
            self._reset_rate_for_each_cores()
            self.timer_mutex.acquire()
